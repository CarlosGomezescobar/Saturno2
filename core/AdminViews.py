...
from SaturnoApp.settings import BASE_URL
    ...
    def get_queryset(self):
        ...
        if filter_val != "":
            cat = Categories.objects.filter(
                Q(title__contains=filter_val) | Q(
                    description__contains=filter_val)
            ).order_by(order_by)
        else:
    ...
    def get_queryset(self):
        ...
        if filter_val != "":
            cat = MerchantUser.objects.filter(
                Q(auth_user_id__first_name__contains=filter_val)
                | Q(auth_user_id__last_name__contains=filter_val)
                | Q(auth_user_id__email__contains=filter_val)
                | Q(auth_user_id__username__contains=filter_val)
            ).order_by(order_by)
        else:
...
class ProductView(View):
    def get(self, request, *args, **kwargs):
        ...
        return render(
            request,
            "admin_templates/product_create.html",
    ...
    def post(self, request, *args, **kwargs):
        ...
        for title_title in title_title_list:
            product_details = ProductDetails(
                title=title_title,
                title_details=title_details_list[j],
                product_id=product,
            )
    ...
    def get_queryset(self):
        ...
        if filter_val != "":
            products = Products.objects.filter(
                Q(product_name__contains=filter_val)
                | Q(product_description__contains=filter_val)
            ).order_by(order_by)
        else:
        ...
        for product in products:
            product_media = ProductMedia.objects.filter(
                product_id=product.id, media_type=1, is_active=1
            ).first()
...
class ProductEdit(View):
    def get(self, request, *args, **kwargs):
        ...
        return render(
            request,
            "admin_templates/product_edit.html",
            {
...
class ProductAddMedia(View):
    def get(self, request, *args, **kwargs):
        ...
        product = Products.objects.get(id=product_id)
        return render(
            request, "admin_templates/product_add_media.html", {
                "product": product}
        )
...
class ProductEditMedia(View):
    def get(self, request, *args, **kwargs):
        ...
        return render(
            request,
            "admin_templates/product_edit_media.html",
...
class ProductMediaDelete(View):
    def get(self, request, *args, **kwargs):
        ...
        os.remove(
            ...
            + str(product_media.media_content).replace("/", "\\")
        )

        product_id = product_media.product_id.id
        product_media.delete()
        return HttpResponseRedirect(
            reverse("product_edit_media", kwargs={"product_id": product_id})
        )
...
class ProductAddStocks(View):
    def get(self, request, *args, **kwargs):
        ...
        product = Products.objects.get(id=product_id)
        return render(
            request, "admin_templates/product_add_stocks.html", {
                "product": product}
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        product_transaction = ProductTransaction(
            ...
            transaction_product_count=new_instock,
            transaction_description="New Product Added",
            transaction_type=1,
        )
        product_transaction.save()
        return HttpResponseRedirect(
            reverse("product_add_stocks", kwargs={"product_id": product_id})
        )
...
class ClienteUserListView(ListView):
    model = Cliente
    template_name = "admin_templates/cliente_list.html"
    paginate_by = 8
    ...
    def get_queryset(self):
        ...
        order_by = self.request.GET.get("orderby", "id")
        if filter_val != "":
            cat = Cliente.objects.filter(

                Q(id__id__contains=filter_val)

                | Q(id__first_name__contains=filter_val)
                | Q(id__last_name__contains=filter_val)
                | Q(id__email__contains=filter_val)
                | Q(id__country__contains=filter_val)
            ).order_by(order_by)
        else:
            cat = Cliente.objects.all().order_by(order_by)

        return cat

    def get_context_data(self, **kwargs):
        context = super(ClienteUserListView, self).get_context_data(**kwargs)
        context["filter"] = self.request.GET.get("filter", "")
        context["orderby"] = self.request.GET.get("orderby", "id")
        context["all_table_fields"] = Cliente._meta.get_fields()
        return context


class ClienteUserCreateView(SuccessMessageMixin, CreateView):
    template_name = "admin_templates/cliente_create.html"
    model = Cliente
    ...
    def form_valid(self, form):
        ...
        cliente.profile_pic = profile_pic_url
        cliente.save()
        messages.success(self.request, "Cliente User Created")
        return HttpResponseRedirect(reverse("cliente_list"))


class ClienteUserUpdateView(SuccessMessageMixin, UpdateView):
    template_name = "admin_templates/cliente_update.html"
    model = Cliente
    fields = ["id", "first_name", "last_name", "email", "country",
              "phone_number", "profile_pic", "country", "city", "direction"]
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        clienteuser = Cliente.objects.get(id=self.object.pk)
        context["ClienteUser"] = clienteuser
        return context

    def form_valid(self, form):

        # Saving Custom User Object for Client User
        cliente = form.save(commit=False)
        cliente.save()
        ...
        cliente.save()
        messages.success(self.request, "Cliente User Updated")
        return HttpResponseRedirect(reverse("cliente_list"))


# def export_excel(request):
#     response = HTTPResponse(content_type='application/ms-excel')
#     response['Content-Dispostion'] = 'attachment; filename=Expenses' + \
...
#     rows = Cliente.objects.filter(
#         owner=request.user).values_list('id', 'url_slug')
